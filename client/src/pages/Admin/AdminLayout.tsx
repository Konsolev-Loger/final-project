...
export const AdminLayout: React.FC<Props> = ({ children, loading }) => {
  ...
  return (
    <section className="min-h-screen relative flex items-center justify-center overflow-hidden bg-gradient-to-br from-primary/20 to-primary/10">
      ...
      <div className="container mx-auto px-4 py-12 relative z-20 w-full max-w-7xl">
        ...
        <button
          onClick={() => navigate('/')}
          className="fixed top-4 right-4 z-50 px-6 py-3 bg-white/80 backdrop-blur-md border border-primary rounded-2xl text-primary font-semibold shadow-lg hover:bg-primary/90 hover:text-white transition-all"
        >
          ← На главную
        </button>
